...
RaceKBL removes an offset plus a strictly linear term in time from the samples, then subtracts
...
RaceKBL selects candidate periods by the greedy residual method described above, then fits all of
...
\end{flalign}
In \eqref{eq:racekbl:reconstruction}, \(\omega_m\) are the \(M\) angular frequencies selected by the
