...
RaceKBL removes an affine trend (offset plus linear component) from the samples, then subtracts
the mean of the detrended series. For the periodogram, the model is fit against actual timestamps.
...
sets \(M \in [1, 5]\), which controls how many of the strongest periodogram peaks are passed into
the joint fit. A smaller \(M\) yields a simpler, more stable reconstruction, while a larger \(M\)
captures more structure but risks fitting transient noise. The overlay and fit score update
immediately when the order changes.

The reconstructed signal can be written as:

\begin{flalign}
\hspace{5em}\hat{x}(t) &= \sum_{m=1}^{M}\left(a_m\cos\left(\omega_m(t-\tau_m)\right) + b_m\sin\left(\omega_m(t-\tau_m)\right)\right) + \gamma + \beta t && \left[u\right] \label{eq:racekbl:reconstruction}
\end{flalign}
In \eqref{eq:racekbl:reconstruction}, \(\omega_m\) are the top \(M\) angular frequencies ranked by
periodogram power, \(a_m\) and \(b_m\) are the jointly fitted cosine and sine coefficients, \(\tau_m\)
are the phase offsets used to align each basis function, \(\gamma\) is the constant offset that
combines the detrended mean with the affine intercept, and \(\beta t\) is the strictly linear
through-origin trend component that was removed before the spectral analysis.
This overlay shares the same history window as the wind speed plot, so it updates whenever new
samples arrive or the history slider changes.
