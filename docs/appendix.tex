...
used for ordering, and a typed payload. A short in-memory queue batches writes to avoid
per-event overhead, and a prune pass keeps roughly the last hour of data. The result is a
ring buffer that is offline-friendly yet detailed enough for later analysis.
...
containing the full coordinate payload plus both GPS and device time, \texttt{imu} records
containing the raw motion samples, \texttt{kalman} records containing the filter state, and
\texttt{derived} records containing the processed boat state (position, velocity, speed,
heading). This mix allows offline analysis to compare raw inputs against what the onboard
filter produced without recomputing everything from scratch.

When the user presses the send button, the app flushes pending writes, reads the last hour
from IndexedDB, prepends an \texttt{upload} record with a build stamp, and sorts by timestamp.
The payload is serialized as NDJSON so each record remains independent, then compressed with
gzip before posting to the diagnostics endpoint. The line-oriented format keeps the payload
streamable and lets the server store the data without parsing it into a larger schema. If
the endpoint expects an upload key, the app includes it as a bearer token so only authorized
clients can post data.
...
and inspects the recording to see whether raw GPS and IMU streams are present. If they are,
those records drive the replay; if not, the system falls back to \texttt{derived} samples.
Each record carries a device-time offset from the start of the file, and optional sensor
times (GPS time or motion-event time) are converted into matching offsets so the original
