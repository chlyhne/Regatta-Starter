...
When debug mode is enabled, RaceLifter reads heading values from
\texttt{debug\_wind\_data.csv} and replays them as if they were live measurements. The
replay speed slider adjusts the playback time base, and the render rate is capped to the
same per-second cadence used for live data so the UI remains stable. This makes it
possible to tune the plot behavior and scaling without being on the water.
