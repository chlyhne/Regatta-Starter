...
function normalizeReturnView(value) {
  ...
  if (!view) return "line";
  return RETURN_VIEWS.has(view) ? view : "line";
}

function buildReturnHref() {
  const params = new URLSearchParams(window.location.search);
  const view = normalizeReturnView(params.get("return"));
  return `index.html${getNoCacheQuery()}#${view}`;
}
...
function loadSettings() {
  ...
  state.sessionSet.a =
    Number.isFinite(state.line.a?.lat) && Number.isFinite(state.line.a?.lon);
  state.sessionSet.b =
    Number.isFinite(state.line.b?.lat) && Number.isFinite(state.line.b?.lon);
}
...
function updateMapOverlays() {
  ...
  if (hasA) {
    ...
    if (!state.markerA) {
      state.markerA = L.circleMarker([state.line.a.lat, state.line.a.lon], portStyle).addTo(
        state.map
      );
    } else {
  ...
  if (hasB) {
    ...
    if (!state.markerB) {
      state.markerB = L.circleMarker([state.line.b.lat, state.line.b.lon], starboardStyle).addTo(
        state.map
      );
    } else {
  ...
  if (hasA && hasB) {
    ...
    if (lineLen >= 1) {
      ...
      const stemLatLngs = [fromMeters(mid, origin), fromMeters(base, origin)];
      const headLatLngs = [fromMeters(tip, origin), fromMeters(left, origin), fromMeters(right, origin)];
      const labelPoint = {
...
function bindEvents() {
  ...
  if (els.closeMap) {
    els.closeMap.addEventListener("click", close);
