...
For the periodogram, the model is fit against actual timestamps.
...
\begin{flalign}
\hspace{5em}\hat{x}(t) &= \sum_{m=1}^{M}\left(a_m\cos\left(\omega_m(t-\tau_m)\right) + b_m\sin\left(\omega_m(t-\tau_m)\right)\right) + \mu + \left(\alpha + \beta t\right) && \left[u\right] \label{eq:racekbl:reconstruction}
\end{flalign}
...
are the phase offsets used to align each basis function, \(\mu\) is the mean of the detrended
series, and \(\alpha + \beta t\) is the linear trend that was removed before the spectral analysis.
