...
Note: LaTeX version maintained in `docs/kalman.tex` and compiled via `docs/master.tex`.
...
assumption directly to the code. Tuning details live in `docs/tuning.tex`.
...
- $q$ is the **acceleration variance** (units $(\mathrm{m/s^2})^2$)
...
For one axis (say $x$), with state $[p, v]^T$, constant velocity model, and white acceleration noise,
...
independent copies ($x$ and $y$), combined into $4\times4$ form:
$$
Q =
\begin{bmatrix}
...
Browsers provide `position.coords.accuracy` in meters. It is (roughly) a $1\sigma$ radius. We make a
...
- good GPS (small accuracy) $\Rightarrow$ small $R$ $\Rightarrow$ higher gain $\Rightarrow$ filter follows measurements more
- bad GPS (large accuracy) $\Rightarrow$ large $R$ $\Rightarrow$ lower gain $\Rightarrow$ filter trusts the model more
...
- Every $\approx 200\,\mathrm{ms}$ we advance the state with the motion model using **elapsed time**.
...
The low-pass factor is configured in `docs/tuning.tex` along with the other tuning
constants. This section focuses on how the estimate is used once tuned.
...
$A$, the correct covariance is $P' = A P A^T$. Not rotating $P$ would leave the filter
...
velocity. If IMU is enabled, GPS nudges the IMU heading with a tunable weight. If IMU is
disabled, GPS heading is used directly. Tuning details live in `docs/tuning.tex`.
...
## 10) Why `Q` is rotated: forward vs sideways acceleration
...
sideways variance, then **rotating that covariance into the global x/y frame** using the
current velocity heading. This keeps the model aligned with the boat’s direction of travel
and makes the filter less willing to “invent” sideways speed.
...
- Tuning reference: `docs/tuning.tex` (values live in `tuning.js`)
