...
centerline filled in the same color. The center line marks zero deviation, and vertical
grid lines mark every \(5^\circ\) of deviation.
...
RaceLifter always uses the Kalman-derived heading, matching the pipeline used in
RacePerformance. The device motion sensor toggle controls whether yaw-rate integration is
blended into the Kalman heading between GPS updates. Regardless of that choice, the
lifter only updates when the reported speed is above the minimum threshold, keeping
low-speed noise from dominating the baseline.
