...
\section{Kalman filter}\label{kalman-filter-math-physical-arguments-and-gain-scheduling}}
...
In RaceTimer we make a more boat-like assumption: forward and sideways
acceleration are not equally likely. We therefore:
...
\hypertarget{vmg-hunter-method-approximate-upwind-vmg}{%
\subsection{VMG Hunter method (approximate upwind
VMG)}\label{vmg-hunter-method-approximate-upwind-vmg}}

VMG Hunter works without a wind sensor. We assume the boat is sailing
upwind and the true wind angle (TWA) stays within a narrow range
(roughly \(35^\circ\) to \(50^\circ\)). The user selects an approximate
TWA, and we use that to decide whether a small change in heading should
be a \emph{bear away} or \emph{head up}.
...
\(\frac{dv}{d\psi}\) from GPS data and compare it to the break-even
slope. If the estimated slope is greater than the break-even slope,
bearing away is faster; if it is smaller, heading up is faster. Because
our heading samples are in degrees, the comparison uses:
\[
\frac{dv}{d(\mathrm{deg})} = v\tan\psi \cdot \frac{\pi}{180}
\]

Estimating the slope from GPS. We use recursive least squares (RLS) with
exponential forgetting so it behaves like an IIR filter and responds
smoothly:
\begin{itemize}
\tightlist
\item
  use GPS speed and heading (or compute them from successive positions)
\item
  unwrap heading to avoid \(360^\circ\) discontinuities
\item
  update the RLS state \((\theta, P)\) with a forgetting factor set by
  \(\tau \approx 10\,\mathrm{s}\)
\end{itemize}

Heading source. When IMU is enabled, we integrate the gyro yaw rate to
track heading changes and use that heading in the regressor. GPS course
is used to correct long-term drift only when speed exceeds
\(2\,\mathrm{kn}\) and GPS accuracy is acceptable. If IMU is disabled, or
no IMU heading is available yet, we fall back to GPS heading.

Let \(h_k\) be heading (degrees) and \(v_k\) speed (m/s) at sample \(k\).
We use unwrapped heading relative to a fixed reference so the regressor
stays small: \(h_k = \psi_k - \psi_0\). This shift does not change the
slope. We fit a line \(v_k = b_k + s_k h_k + e_k\), written in vector form:
\[
...
With forgetting factor \(\lambda_k = e^{-\Delta t_k/\tau}\), the RLS
recursion is:
\[
...
This is the standard RLS form; the implementation follows this
recursion directly.
...
\]
where \(\sigma_{e,k}^2\) is an exponentially weighted residual variance,
for example:
\[
\sigma_{e,k}^2 = (1-\alpha_k)\sigma_{e,k-1}^2 + \alpha_k e_k^2,\quad
\alpha_k = 1 - e^{-\Delta t_k/\tau}
\]
The UI maps \(\sigma_{s,k}\) to a confidence band width. If GPS is stale or
accuracy is poor, the bar is desaturated to signal low trust.

Tack handling. The indicator is directional: on port tack the left/right
meaning flips. We implement this by negating the slope before comparing
to the break-even value and swapping the labels accordingly.
